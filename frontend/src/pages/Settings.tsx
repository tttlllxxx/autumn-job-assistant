...
type SystemStatus = { active_jobs: number; applications: number; recommendation_version: number | null; recommendation_updated_at: string | null; source_status: Record<string, number>; task_status: Record<string, number>; recent_tasks: Array<{ id: number; task_type: string; status: string; message: string; error_message: string | null }> };
...
export function Settings() {
  ...
  const systemStatus = useQuery<SystemStatus>({ queryKey: ["system-status"], queryFn: () => api("/api/system/status"), refetchInterval: 10_000 });
  ...
  return <>
    ...
    <AsyncState loading={budget.isLoading || preferences.isLoading || llmConfig.isLoading} error={(budget.error ?? preferences.error ?? llmConfig.error) as Error | null}>
      ...
      <section className="panel"><p className="panel-kicker">推荐质量</p><h2>精度样本</h2>{evaluation.isLoading ? <p>统计中…</p> : evaluation.error ? <p className="error-text">{evaluation.error.message}</p> : <><p>已判断 {evaluation.data?.labels ?? 0} / {evaluation.data?.required_labels ?? 50} 个岗位</p><progress max={evaluation.data?.required_labels ?? 50} value={evaluation.data?.labels ?? 0} aria-label="推荐精度样本收集进度" />{evaluation.data?.precision_at_10 == null ? <><p className="muted">继续在岗位详情选择“符合/不符合”；当前 Top10 也必须全部判断。</p>{evaluation.data?.unlabeled_top10_job_ids?.[0] && <Link to={`/jobs/${evaluation.data.unlabeled_top10_job_ids[0]}`}>判断下一个未标注的 Top10 岗位 →</Link>}</> : <p className={evaluation.data.precision_at_10 >= .7 ? "success" : "warning"}>Precision@10：{(evaluation.data.precision_at_10 * 100).toFixed(0)}%</p>}</>}</section></div>
    </AsyncState>
    ...
    <section className="panel"><p className="panel-kicker">运行状态</p><h2>流水线概览</h2>{systemStatus.isLoading ? <p>读取中…</p> : systemStatus.error ? <p className="error-text">{systemStatus.error.message}</p> : <><div className="score-grid"><article><span>有效岗位</span><strong>{systemStatus.data?.active_jobs ?? 0}</strong></article><article><span>投递记录</span><strong>{systemStatus.data?.applications ?? 0}</strong></article><article><span>推荐版本</span><strong>{systemStatus.data?.recommendation_version ?? "—"}</strong></article><article><span>失败任务</span><strong>{(systemStatus.data?.task_status?.failed ?? 0) + (systemStatus.data?.task_status?.interrupted ?? 0)}</strong></article></div><p className="muted">来源：{Object.entries(systemStatus.data?.source_status ?? {}).map(([status, count]) => `${status} ${count}`).join(" · ") || "尚无运行记录"}</p>{systemStatus.data?.recent_tasks?.map((task) => <div className="row-link" key={task.id}><span>{task.task_type}<small>{task.message}</small></span><span className={task.status === "failed" || task.status === "interrupted" ? "error-text" : ""}>{task.status}{task.error_message ? ` · ${task.error_message}` : ""}</span></div>)}</>}</section>
