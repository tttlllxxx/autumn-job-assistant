...
export function Applications() {
  ...
  return <>
    ...
    <AsyncState loading={items.isLoading} error={items.error}><div className="board">{statuses.map((status) => { const columnItems = items.data?.items.filter((item) => item.status === status) ?? []; return <section key={status}><div className="board-column-title"><h2>{status}</h2><span>{columnItems.length}</span></div>{columnItems.length ? columnItems.map((item) => <article key={item.id} className={item.id < 0 ? "saving" : ""}><div className="application-card-head"><span className="company-mark">{item.position.slice(0, 1)}</span><div><div className="application-title-line"><strong>{item.position}</strong><span>{item.status} → {item.current_stage} · {item.stage_result}</span></div><p>{item.company}</p></div></div>{item.base_location && <small className="application-meta">⌖ {item.base_location}</small>}<label>状态<select aria-label={`${item.position} 状态`} value={item.status} disabled={item.id < 0} onChange={(event) => update.mutate({ id: item.id, changes: { status: event.target.value } })}>{statuses.map((value) => <option key={value}>{value}</option>)}</select></label><label>阶段<select aria-label={`${item.position} 当前阶段`} value={item.current_stage} disabled={item.id < 0} onChange={(event) => update.mutate({ id: item.id, changes: { current_stage: event.target.value } })}>{stages.map((value) => <option key={value}>{value}</option>)}</select></label><label>结果<select aria-label={`${item.position} 阶段结果`} value={item.stage_result} disabled={item.id < 0} onChange={(event) => update.mutate({ id: item.id, changes: { stage_result: event.target.value } })}>{results.map((value) => <option key={value}>{value}</option>)}</select></label>{item.id < 0 && <small className="saving-label">正在保存…</small>}</article>) : <p className="board-empty">暂无记录</p>}</section>; })}</div></AsyncState>
  </>;
}
