...
export function Setup() {
  ...
  const upload = useMutation({ mutationKey: taskKeys.resumeParse, mutationFn: async () => { if (!file) throw new Error("请选择文件"); const form = new FormData(); form.append("file", file); return api<Resume>("/api/resumes", { method: "POST", body: form }); }, onSuccess: () => { void client.invalidateQueries({ queryKey: ["resumes"] }); void client.invalidateQueries({ queryKey: ["profile"] }); } });
  const uploadPending = useTaskPending(taskKeys.resumeParse);
  return <><header className="page-head"><div><p className="eyebrow">ONBOARDING</p><h1>导入原始简历</h1><p>支持可复制文本的 PDF 或 UTF-8 Markdown，最多 10 MB。</p></div></header>
    <section className="panel"><form onSubmit={(event) => { event.preventDefault(); upload.mutate(); }}><label htmlFor="resume">简历文件</label><input id="resume" type="file" accept=".pdf,.md,.markdown,application/pdf,text/markdown" onChange={(e) => setFile(e.target.files?.[0] ?? null)} /><button disabled={!file || uploadPending}>{uploadPending ? "解析中…" : "上传并解析"}</button>{upload.error && <p role="alert" className="error-text">{upload.error.message}</p>}</form></section>
