...
7. 在“投递看板”中以岗位为单位跟踪状态、当前阶段、下一步行动和截止时间。
...
| 03 投递看板 | 在应用内部实时跟踪求职进度 | 一岗一行、筛选排序、行内更新与展开详情 |
...
2. **添加公司来源**：填写公司名称和 HTTPS 官方招聘入口，系统会优先识别 Moka、Greenhouse、Lever、Ashby 的公开岗位接口，并在之后的每日任务中继续更新；其他官网使用页面及浏览器响应解析。
3. **手工录入官方 JD**：当页面需要登录、验证码或自动采集失败时，粘贴官方岗位链接和完整 JD。

每张来源卡会显示采集方式、有效岗位数、上次发现/接受/拒绝数量、更新时间和失败原因。自定义来源被移除后不会再自动更新，但已经采集的历史岗位仍会保留。系统不会绕过登录、验证码或网站风控。

### 岗位推荐与反馈

- “推荐”显示通过硬条件的岗位；“待确认”表示招聘类型或毕业年份等信息仍需人工确认；“已过滤”会显示未通过原因。
- 目标毕业年份和招聘类型在“画像与事实”中配置；修改后需重新确认画像。LLM 只重排本地初筛 Top N，默认 50 个，其余岗位保留透明的本地分数。
...
投递看板采用岗位中心的进度表：每个岗位一行，可搜索、筛选和排序，并直接更新状态、当前阶段、阶段结果、下一步行动和截止时间。展开岗位后可继续维护联系人、面试时间、内推码、结果与备注；所有修改均在应用内部实时保存。
...
| `LLM_RERANK_LIMIT` | 每次进入 LLM 重排的本地 Top N，默认 `50` |
| `FEISHU_WEBHOOK` | 可选的个人飞书机器人 Webhook |
| `SCHEDULE_HOUR` | 每日任务执行小时，默认上海时区 `8` 点 |

## 自动任务与飞书通知

应用按 `Asia/Shanghai` 时区每天 08:00 依次执行采集、推荐和飞书通知；可通过 `SCHEDULE_HOUR` 修改整点小时。画像尚未确认时只执行采集；岗位、画像和反馈均无变化时会跳过重复推荐计算。任务状态保存在数据库中，换页或刷新后仍可恢复，并在完成时给出全局提示。
...
- “设置与备份 → 流水线概览”会显示有效岗位、来源健康、推荐版本和最近任务；`/health/ready` 也会检查关键迁移字段是否存在。
